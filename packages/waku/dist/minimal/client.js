...
'use client';
import { createContext, createElement, memo, use, useCallback, useEffect, useState, Component } from 'react';
...
const mergeElementsPromise = (a, b)=>{
    const getResult = ()=>{
        const p = Promise.all([
            erroredElementsPromiseMap.get(a) || a,
            b
        ]).then(([a, b])=>{
            const nextElements = {
                ...a,
...
export const Root = ({ initialRscPath, initialRscParams, fetchCache = defaultFetchCache, unstable_enhanceFetch, unstable_enhanceCreateData, children })=>{
    ...
    const refetch = useCallback((rscPath, rscParams)=>{
...
const InnerSlot = ({ id, children, setValidElement, unstable_fallback })=>{
    ...
    render() {
        ...
        if (error !== null) {
            if (this.props.errorHandler) {
                return createElement(ErrorContextProvider, {
                    value: [
                        error,
                        this.reset
                    ]
 ...
 * ```
 */ export const Slot = ({ id, children, unstable_handleError, unstable_fallback })=>{
    const [errorHandler, setErrorHandler] = useState();
    const setValidElement = useCallback((element)=>setErrorHandler(createElement(ChildrenContextProvider, {
            value: unstable_handleError
        }, element)), [
        unstable_handleError
    ]);
    if (unstable_handleError !== undefined) {
        return createElement(GeneralErrorHandler, {
            errorHandler
    ...
    return createElement(InnerSlot, {
        id,
        unstable_fallback
    }, children);
};
 ...
 * ServerRoot for SSR
 * This is not a public API.
 */ export const INTERNAL_ServerRoot = ({ elementsPromise, children })=>createElement(ElementsContext.Provider, {
