...
type IsPageWithSlug<Page extends AnyPage> = Page extends {
    path: infer P;
} ? P extends PathWithoutSlug<P> ? false : true : never;
/** Used to replace a single slug param [mySlug] with a union of possible slugs 'foo' | 'bar' */
type ReplaceSlugSet<Path extends string, Slugs extends string> = Slugs extends unknown ? ReplaceAll<Path, `[${string}]`, Slugs> : never;
/**
 ...
 */
type ReplaceHelper<SplitPath extends readonly string[], StaticSlugs extends readonly string[], SlugCountArr extends null[] = [], Result extends string[] = []> = SplitPath extends [
    infer PathPart extends string,
    ...infer Rest extends string[]
] ? PathPart extends `[...${string}]` ? [...Result, ...StaticSlugs] : PathPart extends `[${string}]` ? ReplaceHelper<Rest, StaticSlugs, [
    ...SlugCountArr,
    null
], [
    ...Result,
    StaticSlugs[SlugCountArr['length']]
]> : ReplaceHelper<Rest, StaticSlugs, SlugCountArr, [...Result, PathPart]> : Result;
/**
 ...
 */
type ReplaceTupleStaticPaths<Path extends string, StaticPathSet extends readonly string[]> = StaticPathSet extends unknown ? Join<ReplaceHelper<Split<Path, '/'>, StaticPathSet>, '/'> : never;
...
type CollectPathsForStaticSlugPage<Page extends StaticSlugPage> = Page extends {
    ...
    render: 'static';
    staticPaths: infer StaticPaths extends readonly string[] | ReadOnlyStringTupleList;
} ? StaticPaths extends readonly string[] ? ReplaceSlugSet<Path, StaticPaths[number]> : StaticPaths extends ReadOnlyStringTupleList ? ReplaceTupleStaticPaths<Path, StaticPaths[number]> : never : never;
...
type CollectPathsForDynamicSlugPage<Page extends DynamicPage> = Page extends {
    path: infer Path extends string;
} ? ReplaceAll<Path, `[${string}]`, string> : never;
/**
 ...
 *   (e.g., `/[slug]` => `/${string}`)
 */
export type CollectPaths<EachPage extends AnyPage> = EachPage extends unknown ? IsPageWithSlug<EachPage> extends true ? EachPage extends StaticSlugPage ? CollectPathsForStaticSlugPage<EachPage> : EachPage extends DynamicPage ? CollectPathsForDynamicSlugPage<EachPage> : never : EachPage['path'] : never;
 ...
 */
export type PathsForPages<PagesResult extends {
    DO_NOT_USE_pages: AnyPage;
} | AnyPage> = PagesResult extends {
    DO_NOT_USE_pages: AnyPage;
} ? CollectPaths<PagesResult['DO_NOT_USE_pages']> extends never ? string : CollectPaths<PagesResult['DO_NOT_USE_pages']> : PagesResult extends AnyPage ? CollectPaths<PagesResult> extends never ? string : CollectPaths<PagesResult> : never;
type _GetSlugs<Route extends string, SplitRoute extends string[] = Split<Route, '/'>, Result extends string[] = []> = SplitRoute extends [] ? Result : SplitRoute extends [`${infer MaybeSlug}`, ...infer Rest extends string[]] ? MaybeSlug extends `[${infer Slug}]` ? _GetSlugs<Route, Rest, [...Result, Slug]> : _GetSlugs<Route, Rest, Result> : Result;
...
type IndividualSlugType<Slug extends string> = Slug extends `...${string}` ? string[] : string;
type CleanWildcard<Slug extends string> = Slug extends `...${infer Wildcard}` ? Wildcard : Slug;
...
export type PropsForPages<Path extends string> = Prettify<Omit<RouteProps<ReplaceAll<Path, `[${string}]`, string>>, 'hash'> & SlugTypes<Path>>;
type GetResponseType<Response extends {
    render: string;
}> = string extends Response['render'] ? {
    render: 'dynamic';
} : Response;
/**
 ...
 */
export type GetConfigResponse<Fn extends () => Promise<{
    render: string;
}> | {
    render: string;
}> = ReturnType<Fn> extends {
    render: string;
} ? GetResponseType<ReturnType<Fn>> : GetResponseType<Awaited<ReturnType<Fn>>>;
export {};
