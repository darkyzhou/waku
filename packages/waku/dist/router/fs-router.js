import { unstable_getPlatformData, unstable_setPlatformData, unstable_getBuildOptions } from '../server.js';
...
export function unstable_fsRouter(importMetaUrl, loadPage, options) {
    ...
    return createPages(async ({ createPage, createLayout, createRoot, createApi, createPagePart })=>{
        ...
        if (!files) {
            // dev and build only
            if (import.meta.env && import.meta.env.MODE === 'production' && !buildOptions.unstable_phase) {
