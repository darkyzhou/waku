...
export const emitPlatformData = async (distDir)=>{
    ...
    distEntriesFileContent = distEntriesFileContent.replace('globalThis.__WAKU_LOAD_PLATFORM_DATA__', `
(key) => {
  switch (key) {
    ${Array.from(keys).map((k)=>`case '${k}': return import('./${DIST_PLATFORM_DATA}/${k}.js').then((m) => m.default);`).join('\n')}
