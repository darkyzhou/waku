...
function separatePublicAssetsFromFunctions({ outDir, functionDir, assetsDir }) {
    const tempDist = path.join(os.tmpdir(), `dist_${randomBytes(16).toString('hex')}`);
...
export function deployCloudflarePlugin(opts) {
    ...
    return {
        ...
        async closeBundle () {
            ...
            if (!existsSync(wranglerTomlFile) && !existsSync(wranglerJsonFile) && !existsSync(wranglerJsoncFile)) {
                writeFileSync(wranglerJsonFile, `
{
  "name": "waku-project",
...
//# sourceMappingURL=vite-plugin-deploy-cloudflare.js.map