...
export const fsRouterTypegenPlugin = (opts)=>{
    ...
    return {
        ...
        async configResolved (config) {
            ...
            entriesFilePossibilities = EXTENSIONS.map((ext)=>joinPath(config.root, opts.srcDir, SRC_ENTRIES + ext));
            outputFile = joinPath(config.root, opts.srcDir, 'pages.gen.ts');
        },
        configureServer (server) {
            if (!entriesFilePossibilities || !pagesDir || !outputFile || entriesFilePossibilities.some((entriesFile)=>existsSync(entriesFile)) || !existsSync(pagesDir)) {
                return;
            }
            ...
            const fileExportsGetConfig = (filePath)=>{
                if (!pagesDir) {
                    return false;
                }
                ...
                return file.body.some((node)=>{
                    if (node.type === 'ExportNamedDeclaration') {
                        return node.specifiers.some((specifier)=>specifier.type === 'ExportSpecifier' && !specifier.isTypeOnly && (!specifier.exported && specifier.orig.value === 'getConfig' || specifier.exported?.value === 'getConfig'));
                    }
                    return node.type === 'ExportDeclaration' && (node.declaration.type === 'VariableDeclaration' && node.declaration.declarations.some((decl)=>decl.id.type === 'Identifier' && decl.id.value === 'getConfig') || node.declaration.type === 'FunctionDeclaration' && node.declaration.identifier.value === 'getConfig');
                });
            };
            const generateFile = (filePaths)=>{
                const fileInfo = [];
                ...
                for (const filePath of filePaths){
                    ...
                    if (filePath.endsWith('/_layout.tsx') || isIgnoredPath(filePath.split('/'))) {
                        continue;
...
//# sourceMappingURL=vite-plugin-fs-router-typegen.js.map