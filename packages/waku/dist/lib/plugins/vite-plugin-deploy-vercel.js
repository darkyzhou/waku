...
export function deployVercelPlugin(opts) {
    ...
    return {
        ...
        async closeBundle () {
            const { deploy, unstable_phase } = buildOptions;
            if (unstable_phase !== 'buildDeploy' || deploy !== 'vercel-serverless' && deploy !== 'vercel-static') {
                return;
            }
            ...
            if (deploy === 'vercel-serverless') {
                ...
                const serverlessDir = path.join(outputDir, 'functions', opts.rscBase + '.func');
                mkdirSync(path.join(serverlessDir, opts.distDir), {
                    recursive: true
                });
                cpSync(path.join(rootDir, opts.distDir), path.join(serverlessDir, opts.distDir), {
                    recursive: true
                });
                await emitPlatformData(path.join(serverlessDir, opts.distDir));
                if (existsSync(path.join(rootDir, opts.privateDir))) {
                    cpSync(path.join(rootDir, opts.privateDir), path.join(serverlessDir, opts.privateDir), {
                        recursive: true,
                        dereference: true
                    });
                }
            ...
            const routes = deploy === 'vercel-serverless' ? [
                {
                    handle: 'filesystem'
                },
                {
...
//# sourceMappingURL=vite-plugin-deploy-vercel.js.map